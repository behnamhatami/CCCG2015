...
For small $k$, better algorithms are known. For fixed $d$, there exists an $\eps$\nobreakdash-coreset for $1$-center with $\cO(\eps^{-\frac{d-1}{2}})$ space.
For higher dimensions, Agarwal \cite{agarwal2010streaming} gave a $(\frac{1+\sqrt{3}}{2})$-approximation for $1$\nobreakdash-center problem, and Chan \cite{chan2004faster} reduced its approximation factor to $1.22$, by an improved analysis.

...
%For determining outliers, we just need to consider the points that may cause a transition so instead of exiting a node we ignore this point as an outlier. % Sahand: Chi shode? 
%Kiana: benazaram in jomle bayad pak beshe chon ham vazeh ni ham tu badesh tozih dade mishe ghashang 
%Behnam: be nazare man baiad bashe, chon baiad begim bara outlier ha faghat hamin noghteha moheman, in nokte mohemie!
...
% man ba tarife kiana kheili moafegham
\Function{Known$r'$}{$\mathcal{P}, z, r'$}
	\For{$(n_1, \dots, n_4) \gets \set{(n_1, \dots, n_4) \provided \sum n_i=z}$}
		...
		\If{$p_{o_2}$ arrives}
			\State $CS\gets$ the case of $p_{o_2} \in B_{2, 1}^*$ $\cup$ the case of $p_{o_2} \in B_{2,2}^*$.
			\For{each case $S$}
...
\begin{figure}[h]
	...
	\label{fig:2lt1}
\end{figure}
%Behnam: tooie aks namosavit eshtebas baiad be delta/d koochiktar az C/2r* tabdil beshe
...
% Kiana: and removing
% Behnam: we don't need removing a point!
...
\For{$p \in P$}
	...
	\If{!\Call{AddTo$B_1$}{$p$} and !\Call{AddTo$B_2$}{$p$}}
		...
		\While{$\card{\mbox{Buffer}} > z$}
			...
			\EndIf
			\State $T \gets \mbox{Buffer} \cup B_2 - {c_2}$

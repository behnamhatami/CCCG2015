...
First case is in \Cref{alg:addB1}, when  $\len{pc_1} \leq \delta$, which due to variant (a), garanties $\len{pc_1} \le \delta^*$, so $p \not \in B_2^*$.
Second case is in \Cref{alg:case-1}, when the buffer becomes full and $c_2$ has been set before. 
...
If $|B_2| \geq \dz$, then $c_p$ is the centerpoint of the first $\dz$ points currently in $B_2$. in this case, we add all of $B_2$ points to $B_1$. Due to variant (b), $r \leq \delta/2 < \delta^*/2$. So By Lemma \ref{obs:center-point} then $$ By Lemma \ref{lem:} By contradiction assume that $B_2 \cap B_2^* \neq \emptyset$. If $c_p \in B_2^*$, then by variant (b), $B(c_p, \frac{2 \len{c_1 c_p}}{\alpha}) \subset B(c_2, r)$
}
\item [(d)] {
	\begin{enumerate}
		\item [1.]{
	...
	if $|B_2| < \dz$ then $c_p = c_2$ and By \Cref{alg:case-1}, $r = r_0 = \frac{2}{\alpha} \len{c_1 c_2}$, so $2r^* \leq r$.
...
The function \textproc{AddToB$_1$} presented in \Cref{alg:addB1} 
is used to add a point $p$ to $B_1$. 
%The point $c_1$ is the first point of the stream,
%which is assumed to be non-outlier, and is in $B_1^*$.
...
%among the points of $P$ seen so far.
...
The function \textproc{AddToB$_2$} presented in \Cref{alg:addB2}
...
Consider $c_2$ as the central point $c_p$. % is considered as the central point $\cp$. 
...
and hence, property P2 is maintained. %invariant under this operation.


When the size of $B_2$ reaches $\dz$, 
we update the central point $\cp$ to the centerpoint of $B_2$.
%We claim that $B(c_2, (2 + \frac{2}{\alpha})\rc )$ completely covers
%the ball $B(\cp, \frac{2\len{c_1\cp}}{\alpha})$.
...
%by setting $B_2 = B(c_2, (2 + \frac{2}{\alpha})\rc )$, 
...
\begin{lemma}
\label{lem:}
	After termination of \Cref{alg:case-1}, 
	if $|B_2| \ge \dz$ and $\cp \not\in B_2^*$,
...
%1-centers obtained from $B_1$, $B_2$ and $B_u$ by the $\beta$-approximation algorithm, respectively. 
...
%\Function{Query}{}
	...
	\IF {$\card{B_2} < \dz$}
		\STATE candidates $\gets \text{Buffer} \cup B_2$ % \setminus \set{c_2}$
	\ENDIF
...
\begin{tikzpicture}
 ...
 \tikzset{edge/.style = {->,> = latex'}}
  ...
  \foreach \from/\to in {S/1,1/2a,1/3a,2a/2b,2b/2c,2c/E,3a/3b,3a/3b,3a/3d,3b/3c,3d/3e,3c/E,3e/E}
    \draw[edge] (\from) to (\to);
